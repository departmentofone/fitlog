...
import { communitySearchText, useCommunity, useReportCommunityItem, useSaveCommunityItem, type CommunityItem, type CommunityKind } from '../../hooks/useCommunity'
...
const KIND_LABEL: Record<CommunityKind, string> = { meal: 'Meal', recipe: 'Recipe', workout: 'Workout', program: 'Program' }
...
function CommunityCard({ item, saved, unit }: { item: CommunityItem; saved: boolean; unit: UnitSystem | undefined }) {
  const { show } = useToast()
  ...
  function handleSave() {
    ...
    save.mutate(item, {
      onSuccess: () => {
        haptics.success()
        const where = item.kind === 'meal' ? 'Meals → Presets' : item.kind === 'recipe' ? 'Meals → Recipes' : 'Workouts → Presets'
        show(`Saved "${item.name}" - find it in ${where}`)
      },
          ...
          >
            {done ? (
              <>
                ...
                {item.kind === 'program' ? 'Imported' : 'Saved to your account'}
              </>
            ) : busy ? (
              'Saving…'
            ) : (
...
export function CommunityTab() {
  ...
  const savedIds = useMemo(
    () => new Set([...myMeals, ...myRecipes, ...myWorkouts].map((x) => x.source_id).filter((id): id is string => !!id)),
    [myMeals, myRecipes, myWorkouts],
  )
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((i) => (filter === 'all' || i.kind === filter) && (!q || communitySearchText(i).includes(q)))
  }, [items, filter, search])
    ...
    <div className="space-y-4 p-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Meals, recipes and workouts people have shared. Saving makes your own copy - change it however you like.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {visible.map((item) => (
            <CommunityCard key={`${item.kind}-${item.id}`} item={item} saved={savedIds.has(item.id)} unit={settings?.unit_system} />
          ))}
        </div>
      )}
    </div>
